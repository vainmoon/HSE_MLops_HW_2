...
**Надёжность**
- Артефакты моделей не должны теряться.
...
| Артефакты | MinIO | удобный и простой S3 для домашнего компьютера|
| Оркестрация | Docker Compose | простой запуск всех сервисов одной командой, кубер был бы избыточен для такого задания|
...
| POST | `/auth/register`| Зарегистрировать пользователя в бд | 
...
**model_versions** — версия модели. `version_number` инкрементируется в рамках модели. `status`: `staging` → `production` → `archived`. `training_params` и `metrics` — JSONB, конкретные метрики и параметры как схема не фиксируется.
